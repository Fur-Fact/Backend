...
exports.getPetsByUserId = async (req, res) => {
  ...
  try {
    const pets = await petRepository.getPetsByUserId(user.id);
    if (pets.length > 0) {
      res.status(200).send({ result: 'success', pets });
    } else {
      res.status(404).send({ result: 'fail', message: '해당 유저의 펫을 찾을 수 없습니다.' });
    }
  } catch (err) {
